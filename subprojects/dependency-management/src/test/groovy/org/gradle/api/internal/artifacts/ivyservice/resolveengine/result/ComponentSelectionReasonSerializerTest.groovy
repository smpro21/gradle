...
class ComponentSelectionReasonSerializerTest extends SerializerSpec {
    ...
    def "serializes custom reasons"() {
        expect:
        check(VersionSelectionReasons.CONFLICT_RESOLUTION.withDescription("my conflict resolution"))
        check(VersionSelectionReasons.CONFLICT_RESOLUTION_BY_RULE.withDescription("my conflict resolution by rule"))
        check(VersionSelectionReasons.FORCED.withDescription("forced by me"))
        check(VersionSelectionReasons.REQUESTED.withDescription("I really asked for it"))
        check(VersionSelectionReasons.ROOT.withDescription("I know this is the root of the graph"))
        check(VersionSelectionReasons.SELECTED_BY_RULE.withDescription("Wouldn't it be nice to add custom reasons?"))
    }
    ...
    private static ComponentSelectionReasonInternal withReason(String reason) {
        VersionSelectionReasons.SELECTED_BY_RULE.withDescription(reason)
    }
